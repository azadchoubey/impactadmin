...
<div style="max-width: 48rem;" class="max-w-lg mx-auto grid grid-cols-3 gap-3">     
    <label for="simple-search" class="sr-only">Search</label>
        <div class="relative flex-grow">
            ...
            <input autocomplete="off" wire:model="title" wire:keyup.debounce.200ms="updateTitle" type="text" id="simple-search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 pr-4 py-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search publication name..." required>
        ...
        <input wire:model="date" type="date"  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 pr-4 py-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
...
<table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            <a wire:click="" href="javascript:void(0);">{{$result->Title}}</a> 
             
            <td>{{$result->publication->Title}}</td>  
             <td>{{$result->publication->edition->Name}}</td>  
             <td>{{$result->pubdate}}</td>  
             <td><a class="px-3 py-2 text-xs font-medium text-center text-white bg-green-700 rounded-lg hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800" href="{{route('viewarticle',$result->ArticleID)}}" class="">View</a></td>  
            </tr>
            @endforeach
        </tbody>
    </table>
